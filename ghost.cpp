...
int wallCollide(int x, int y, int dir, int speed, int level[31][29]) {
	...
	// Check along the far right side of the sprite, plus 4 (the speed we’re moving)
		new_x1 = x + speed + PACSIZE * 2; //multiplying by 2 since PACSIZE is the radius
		new_x2 = x + speed + PACSIZE * 2;
		new_x3 = x + speed + PACSIZE * 2;
		...
		//Check at three points along that edge
		new_y1 = y + speed + PACSIZE * 2;
		new_y2 = y + speed + PACSIZE * 2;
		new_y3 = y + speed + PACSIZE * 2;
...
void ghost::initGhost(int x, int y, int dir, bool dead, int Gspeed, sf::Color Gcolor) {
	xpos = x;
	ypos = y;
	direction = dir;
	dead = dead;
...
void ghost::chase1(int x, int y, int level[31][29]) {
	...
	if ((direction == LEFT) || (direction == RIGHT)) {
		// Can we turn UP?
		if (!wallCollide(xpos, ypos, UP, speed, level)) {
			// Is the player UP?
			if ((y < ypos)) {
				direction = UP;
			}
		}
		if (!wallCollide(xpos, ypos, DOWN, speed, level)) {
			// Is the player DOWN?
			if ((y > ypos)) {
				direction = DOWN;
			}
	...
	// Moving UP or DOWN, we check for possibility of turning UP or DOWN
		...
		// Can we turn LEFT?
		if (!wallCollide(xpos, ypos, LEFT, speed, level)) {
			// Is the player LEFT?
			if ((x < xpos)) {
				direction = LEFT;
			}
		}
		if (!wallCollide(xpos, ypos, RIGHT, speed, level)) {
			// Is the player RIGHT?
			if ((x > xpos)) {
				direction = RIGHT;
			}
	...
	if (direction == UP && wallCollide(xpos, ypos, UP, speed, level)) { direction = DOWN; }
	else if (direction == DOWN && wallCollide(xpos, ypos, DOWN, speed, level)) { direction = UP; }
	else if (direction == LEFT && wallCollide(xpos, ypos, LEFT, speed, level)) { direction = RIGHT; }
	else if (direction == RIGHT && wallCollide(xpos, ypos, RIGHT, speed, level)) { direction = LEFT; }

	// Once we've decided where to move, do so.
	if (direction == UP) ypos -= 3;
	else if (direction == DOWN) ypos += 3;
	else if (direction == LEFT) xpos -= 3;
	else if (direction == RIGHT) xpos += 3;


}

//chase function. x and y are pacman's position.
void ghost::chase2(int x, int y, int level[31][29]) {
...
//chase function. x and y are pacman's position.
void ghost::chase3(int x, int y, int level[31][29]) {
	...
	if ((direction == LEFT) || (direction == RIGHT)) {
		// Can we turn UP?
		if (!wallCollide(xpos, ypos, UP, speed, level) && rand() % 2 + 1 == 2) {
			direction = UP;
		}
		if (!wallCollide(xpos, ypos, DOWN, speed, level) && rand() % 2 + 1 == 2) {
			direction = DOWN;
		}
	}
	// Moving UP or DOWN, we check for possibility of turning LEFT or RIGHT
		...
		// Can we turn LEFT?
		if (!wallCollide(xpos, ypos, LEFT, speed, level) && rand() % 2 + 1 == 2) {
			direction = LEFT;
		}
		if (!wallCollide(xpos, ypos, RIGHT, speed, level) && rand() % 2 + 1 == 2) {
			direction = RIGHT;
		}
	}
	// Final case, if we bump into a wall.
...
//chase function. x and y are pacman's position.
void ghost::chase4(int x, int y, int level[31][29]) {
	...
	// Once we've decided where to move, do so.
	if (direction == UP) ypos -= speed;
	else if (direction == DOWN) ypos += speed;
	else if (direction == LEFT) xpos -= speed;
	else if (direction == RIGHT) xpos += speed;
